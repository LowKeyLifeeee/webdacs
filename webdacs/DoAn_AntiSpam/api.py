...
def ping():
    return jsonify({'status': 'ok', 'model_loaded': phobert_model is not None, 'version': '2.0'})
...
def predict():
    ...
    for word in BLACKLIST_KEYWORDS:
        # Dùng regex để tìm chính xác từ (tránh việc "alphabet" bị dính chữ "bet")
    ...
    # Bước 3: Nếu DeepSeek lỗi, Fallback về model PhoBERT
    if not phobert_model or not tokenizer:
        return jsonify({'error': 'DeepSeek API lỗi và Mô hình PhoBERT chưa được tải'}), 500
    ...
    is_spam = bool(spam_prob >= 50.0)

    if not is_spam:
        probability = 100 - spam_prob
    else:
...
def predict_post():
    ...
    try:
        ...
        prompt = f"""Bạn là chuyên gia Anti-Spam. Hãy đánh giá độ an toàn của BÀI VIẾT này (kết hợp Hình Ảnh và Văn Bản).

Nội dung bài đăng: "{text_context[:800]}"

Hướng dẫn:
- Đọc chữ trong ảnh và đối chiếu với nội dung bài đăng để hiểu đúng ngữ cảnh.
- Nếu là quảng cáo cờ bạc, lừa đảo, tuyển dụng ảo, trúng thưởng giả mạo -> is_spam = true.
- Nếu là bài viết an toàn, sự kiện game hợp pháp (Liên Quân, eSports), giải trí, mua bán hợp pháp -> is_spam = false.
...
{{
  ...
  "reason": "<Lý do tiếng Việt>"
        ...
        print(f"Lỗi khi gọi predict_post: {e}")
...
def predict_image():
    ...
    try:
        ...
        if not phobert_model or not tokenizer:
            return jsonify({'error': 'DeepSeek Vision lỗi và mô hình PhoBERT chưa được tải'}), 500
        ...
        # Bước 2: Nếu không dính blacklist, dùng model PhoBERT kiểm tra
        inputs = tokenizer(extracted_text.strip(), return_tensors="pt", truncation=True, max_length=256)
        with torch.no_grad():
            outputs = phobert_model(**inputs)
